...
final class DatabaseManager {
    ...
    func insertSchedule(_ schedule: Schedule) throws {
        try dbQueue.sync {
            ...
            if let val = schedule.tagID { sqlite3_bind_text(statement, 19, (val as NSString).utf8String, -1, transient) } else { sqlite3_bind_null(statement, 19) } // tagID binding
            
            sqlite3_bind_int(statement, 20, schedule.isDone ? 1 : 0) // Index adjusted to 20
